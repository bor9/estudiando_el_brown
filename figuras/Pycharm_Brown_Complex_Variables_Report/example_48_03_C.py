...
plt.plot([0, xmax], [0, math.tan(alpha) * xmax], 'r', ms=ms, zorder=9)

bc = 0.4
thetas = np.linspace(0, alpha, 30)
xbc = bc * np.cos(thetas)
ybc = bc * np.sin(thetas)
plt.plot(xbc, ybc, color='k', lw=1)

bc2 = 0.47
plt.text(bc2 * math.cos(alpha/2), bc2 * math.sin(alpha/2)+0.02, r'$\alpha$', fontsize=fontsize, ha='left', va='center')
...
plt.text(xl, yl, r'$C$', fontsize=fontsize, ha='right', va='bottom')
...
# circle labels
dr = 0.05
plt.text(-ytm, r + dr, '$i$', fontsize=fontsize, ha='left', va='bottom')
plt.text(-ytm, -r - dr, '$-i$', fontsize=fontsize, ha='left', va='top')
plt.text(-0.12, xtm, '$0$', fontsize=fontsize, ha='right', va='baseline')

plt.axis('off')

# save as pdf image
plt.savefig('example_48_03_C.pdf', bbox_inches='tight')
