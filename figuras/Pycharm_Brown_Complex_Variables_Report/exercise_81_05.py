...
xc1 = r1 * np.cos(thetas)
yc1 = r1 * np.sin(thetas)
xc2 = -2 + r2 * np.cos(thetas)
...
plt.annotate("", xytext=(xc1[i - 5], yc1[i - 5]), xycoords='data', xy=(xc1[i], yc1[i]), textcoords='data',
             ...
             zorder=10)
plt.text(xc1[i], yc1[i] + 0.1, r'$C:|z|=2$', fontsize=fontsize, ha='left', va='bottom')
i = 50
plt.annotate("", xytext=(xc2[i], yc2[i]), xycoords='data', xy=(xc2[i + 2], yc2[i + 2]), textcoords='data',
             arrowprops=dict(width=0.1, headwidth=5, headlength=10, facecolor='k', edgecolor='k', shrink=0.002),
             zorder=10)
plt.text(xc2[i], yc2[i] + 0.2, r'$C:|z+2|=3$', fontsize=fontsize, ha='center', va='bottom')
...
plt.text(r1 + 0.1, xtm, '$2$', fontsize=fontsize, ha='left', va='baseline')
plt.plot([1, 1], [0, xtl], 'k', lw=1)
plt.text(1 + 0.1, xtm, '$1$', fontsize=fontsize, ha='left', va='baseline')
plt.plot([-2, -2], [0, xtl], 'k', lw=1)
plt.text(-r1 - 0.1, xtm, '$-2$', fontsize=fontsize, ha='right', va='baseline')
plt.plot([-5, -5], [0, xtl], 'k', lw=1)
plt.text(-5 - 0.1, xtm, '$-5$', fontsize=fontsize, ha='right', va='baseline')


plt.axis('off')

# save as pdf image
plt.savefig('exercise_81_05.pdf', bbox_inches='tight')
