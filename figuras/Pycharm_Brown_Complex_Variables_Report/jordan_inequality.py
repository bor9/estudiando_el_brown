...
#
N = 100
x = np.linspace(0, math.pi+0.5, N)
f1 = np.sin(x)
x2 = np.linspace(0, math.pi / 2 + 0.5, N)
f2 = 2 * x2 / math.pi
...
plt.plot(x2, f2, 'k-', lw=1.5)

# etiquetas de los ejes
plt.text(xmax_ax, xtm, r'$\theta$', fontsize=fontsize, ha='center', va='baseline')
...
plt.text(x2[-1] + 0.1, f2[-1], r'$y=\dfrac{2\theta}{\pi}$', fontsize=fontsize, ha='left', va='baseline')
idx = 70
plt.text(x[idx] + 0.2, f1[idx], r'$y=\operatorname{sen}\theta$', fontsize=fontsize, ha='left', va='center')
...
plt.savefig('jordan_inequality.pdf', bbox_inches='tight')
